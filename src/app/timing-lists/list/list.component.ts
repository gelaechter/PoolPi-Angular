    ...
    ngOnInit(): void {
        ...
        switch (this.section) {
            case Section.CHLORINE:
                this.data._chlorineTimings.pipe(distinctUntilChanged()).subscribe((timings: Map<Time, number>) => {
                ...
                break;
            case Section.FILTER:
                this.data._filterTimings.pipe(distinctUntilChanged()).subscribe((timings: Map<Time, Time>) => {
                    this.timingEntries = Object.assign([], Array.from(timings.entries()));
                });
                break;
            case Section.HEATER:
                this.data._heaterTimings.pipe(distinctUntilChanged()).subscribe((timings: Map<Time, Time>) => {
                    this.timingEntries = Object.assign([], Array.from(timings.entries()));
                });
                break;
        }
